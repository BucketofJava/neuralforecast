...
import torch
#Transformer implemented through xtransformers
...
#Because of how neuralforecast does loss I added this, it allows for both doing crossentropy loss and allows for custom loss computation at
#multiple scales without having to mess with how the data is formatted
...
#Converts patches to indices and back
...
class VQVAETokenizer(IndexTokenizer):
    ...
    def get_indices(self, patched_data, index=0):
        ...
        #We dont want tokens to overlap, since these are diff VQVAEs they will have colliding indices
        #so we add the index times the codebook size to make the indices distinct
...
#A simple tokenizer that just takes the mean of the values in a patch and tokenizes as 1 0 -1s, meant for smoke testing and not real imp#
...
class AutoregressiveScaleModel(BaseModel):
    ...
    def __init__(self,
        ...
        #Reversible Ins. Normalization
        self.revin_module=RevIN(1, affine=False)
        #Forecast Horizon
        self.h=h
        #Input Size
        self.input_size=input_size
        #Class of the tokenizer (Mean, VQVAE, etc.)
        self.tokenizer_class=tokenizer_class
        #List of patch sizes
        self.patch_sizes=patch_sizes
        #Smallest patch size, which is what finally gets converted from index -> data
        ...
        #Codebook size of each tokenizer
        self.codebook_size=self.tokenizers[0].codebook_size
        #Number of tokens; sum of the codebook sizes
    ...
    #Training tokenizer method. For now just VQVAE and a pass on the mean one. 
    def train_tokenizer(self, training_data):
        if(self.tokenizer_class!=VQVAETokenizer):
            return
        for i in range(len(self.tokenizers)):
            #Make sure the input data is divisible by the patch size
            ...
            for j in range(1, self.patch_sizes[i]):
                #Rotate the sequence to allow for more robust patching (ie get patches starting at every value mod P)
            ...
            #Train each individual vqvae on the patched training data
    ...
    def forecast_step(self, indices, patch_size, mask_index=1, return_loss=False):
        ...
        #If returning loss (training) then mask and compute the loss
        if(return_loss):
        ...
        #Otherwise just return the new logits
    ...
    def forward(self, windows_batch):
        ...
        for i in range(len(self.patch_sizes)):
            ...
            #Add it to the current sequence of tokens
            if(cur_seq==None):
                indices=tokenized
                cur_seq=indices
            else:
                indices=torch.cat((cur_seq, tokenized), dim=1)
            # If training, run forecast, get loss
            if(self.training):
                ...
                #Otherwise get new indices
                cur_seq=self.transformer_model.generate(indices, seq_len=self.h // self.patch_small)
        #If training we just care about the loss
        if(self.training):
            return tot_loss
        #Get the indices of the final values on the smallest patch size for conversion to real data
        Y=self.tokenizers[-1].indices_to_values(cur_seq[:, -(self.h // self.patch_small):], len(self.tokenizers)-1).flatten(-2, -1)
        #RevIN is disabled for now so I can test more easily - it messes with tokenization
